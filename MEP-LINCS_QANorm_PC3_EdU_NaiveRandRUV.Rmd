...
#Unit of study is array so there are 64 'samples'of 594 spots, no replicates
...
nYEdUList <- lapply(c(2,5,10,20), function(k, YEdU, cIdx, mValue, MValue){
...
#nYEdUkn is the RUV2 normalized EdU responses at different nu and k levels
...
###NaiveRUV2 with Array as Unit of Study

In the following plots naiveRUV-2 normalization is applied to the EdU signal of all spots with an array as the unit of study. The input data is a 64x694 matrix of logit transformed EdU+ values. All spots are designated as negative controls. The faceted plots show various values of the nu and k parameters.
...
p <- ggplot(dt,aes_string(x=groupBy, y=value))+
  geom_boxplot()+ 
  ggtitle(bquote(EdU^{"+"}~ Medians~(m["e,l"]*"-"*m[".,l"])~After~NaiveRUV2~Normalization~with ~Array~Unit~of~Study))+
  xlab("")+ylab("")+
  ylim(-6,4)+
  guides(fill=FALSE)+
  theme_grey(base_size = 12, base_family = "")+
...
p <- ggplot(dt,aes_string(x=groupBy, y=value, colour=colourBy))+
  geom_boxplot()+ 
  ggtitle(bquote(EdU^{"+"}~ Medians~(m["e,l"]*"-"*m["e.,."])~After~NaiveRUV2~Normalization~with ~Array~Unit~of~Study))+
  xlab("")+ylab("")+
  ylim(-6,4)+
  guides(fill=FALSE)+
  theme_grey(base_size = 12, base_family = "")+
...
In the following plots naiveRUV-2 normalization is applied to the EdU signal of all spots with the plate as the unit of study. This creates an 8x5552 matrix of EdU+ spot values. The 694 spots in the A03 control wells of each plate are designated as negative controls. The faceted plots show normalizatins at various nu and k values.
