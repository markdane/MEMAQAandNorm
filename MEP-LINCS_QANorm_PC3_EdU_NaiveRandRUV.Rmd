...
```
---
title: "MEP-LINCS `r x[["CellLine"]]` `r x[["StainingSet"]]` `r x[["Signal"]]` Quality Analysis"
date: "`r Sys.Date()`"
---
...
In the following plots naiveRUV2 normalization is applied to the EdU signal of all spots with an array as the unit of study forming a 64x694 matrix where all spots are designated as negative controls.
...
In the following plots naiveRUV2 normalization is applied to the EdU signal of all spots with the plate as the unit of study forming an 8x5552 matrix with 694 spots on each plate are designated as negative controls.

```{r}
#Setup data with plate as the unit
#There are 8 'samples' with 694 replicates
#Order the spot level data by plate, well, spot number
#Create a spot assignment that works with rotated B row arrays
l3$RSpot <- l3$Spot
l3$RSpot[grepl("B",l3$Well)] <- 701-l3$RSpot[grepl("B",l3$Well)]
...
```{r}
#Use EdUPlate as the input. It is an 8x5552 matrix.
...
nYEdUPlateList <- lapply(c(2,4,8), function(k, YEdUPlate, cIdx, mValue, MValue){
  nuYEdUList <- lapply(c(1e-2, 1e-1, 1), function(nu, k, YEdU, cIdx, mValue, MValue){
    #browser()
  ...
  vnuYEdUDT <- rbindlist(nuYEdUList)
}, YEdU=YEdU, cIdx=which(grepl("A03",colnames(YEdUPlate))), mValue="mel", MValue="Mel")
nYEdUkn <- rbindlist(nYEdUList)
