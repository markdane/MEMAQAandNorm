...
In the following plots naiveReplicateRUV2 normalization is applied to the EdU signal of all spots with the plate as the unit of study. This creates an 8x5552 matrix of EdU+ spot values. The 694 spots in the A03 control wells of each plate are designated as negative controls. The faceted plots show normalizatins at various nu and k values.
...
#Order the spot level data by plate, well, spot number
#Add in ligand and ECMp names so they will carry through the normalization
#l3$WLSE <- paste(l3$Well, l3$Ligand,l3$RSpot,l3$ECMp,sep="_")
...
```{r}
#Use YEdUPlate as the input. It is an 8x5552 matrix with barcode rownames
#and Well_SpotNumber_ECMp column names.
...
```{r}
nYEdUPlateList <- lapply(c(2,3,4,5), function(k, YEdUPlate, cIdx, mValue, MValue){
  nuYEdUList <- lapply(c(1e-2,1e-1,1), function(nu, k, YEdU, cIdx, mValue, MValue){
    #browser()
    nYEdUm <- medianNaiveRUV2Plate(nu, YEdU, cIdx, k)
    setnames(nYEdUm, "Value", "mel")
    nYEdUM <- madNaiveRUV2Plate(nu, YEdU, cIdx, k)
    setnames(nYEdUM, "Value", "Mel")
    ...
    nYEdU$nu <- nu
    return(nYEdU)
  }, k=k, YEdU=YEdUPlate, cIdx=cIdx, mValue="mel", MValue="Mel")
  vnuYEdUDT <- rbindlist(nuYEdUList)
}, YEdUPlate=YEdUPlate, cIdx=which(grepl("A03",colnames(YEdUPlate))), mValue="mel", MValue="Mel")

nYEdUPlatekn <- rbindlist(nYEdUPlateList)
...
p <- ggplot(dt,aes_string(x=groupBy, y=value))+
  geom_boxplot()+ 
  ggtitle(bquote(EdU^{"+"}~ Medians~(m["e,l"]*"-"*m[".,l"])~After~NaiveRUV2~Normalization~with ~Plate~Unit~of~Study))+
  xlab("")+ylab("")+
  ylim(-6,4)+
  guides(fill=FALSE)+
  theme_grey(base_size = 12, base_family = "")+
...
p <- ggplot(dt,aes_string(x=groupBy, y=value, colour=colourBy))+
  geom_boxplot()+ 
  ggtitle(bquote(EdU^{"+"}~ Medians~(m["e,l"]*"-"*m["e.,."])~After~NaiveRUV2~Normalization~with ~Plate~Unit~of~Study))+
  xlab("")+ylab("")+
  ylim(-6,4)+
  guides(fill=FALSE)+
  theme_grey(base_size = 12, base_family = "")+
