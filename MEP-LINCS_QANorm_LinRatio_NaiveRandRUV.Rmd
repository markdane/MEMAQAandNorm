...
In the following plots naiveRUV-2 normalization is applied to the LinRatio signal of all spots with the plate as the unit of study. This creates an 8x5552 matrix of LinRatio+ spot values. The 694 spots in the A03 control wells of each plate are designated as negative controls. The faceted plots show normalizatins at various nu and k values.
...
```{r}
nYLinRatioPlateList <- lapply(c(2,3,4,5), function(k, YLinRatioPlate, cIdx, mValue, MValue){
  nuYLinRatioList <- lapply(c(1e-2,1e-1,1), function(nu, k, YLinRatio, cIdx, mValue, MValue){
    #browser()
    nYLinRatiom <- medianNaiveRUV2Plate(nu, YLinRatio, cIdx, k)
    setnames(nYLinRatiom, "Value", "mel")
    nYLinRatioM <- madNaiveRUV2Plate(nu, YLinRatio, cIdx, k)
    setnames(nYLinRatioM, "Value", "Mel")
    ...
    nYLinRatio$nu <- nu
    return(nYLinRatio)
  }, k=k, YLinRatio=YLinRatioPlate, cIdx=cIdx, mValue="mel", MValue="Mel")
  vnuYLinRatioDT <- rbindlist(nuYLinRatioList)
}, YLinRatioPlate=YLinRatioPlate, cIdx=which(grepl("A03",colnames(YLinRatioPlate))), mValue="mel", MValue="Mel")

nYLinRatioPlatekn <- rbindlist(nYLinRatioPlateList)
...
dt <- nYLinRatioPlatekn

p <- ggplot(dt,aes_string(x=groupBy, y=value))+
  geom_boxplot()+ 
  ggtitle(bquote(Lineage~Ratio~Medians~(m["e,l"]*"-"*m[".,l"])~After~NaiveRUV2~Normalization~with ~Plate~Unit~of~Study))+
  xlab("")+ylab("")+
  #ylim(-6,4)+
  guides(fill=FALSE)+
  theme_grey(base_size = 12, base_family = "")+
...
p <- ggplot(dt,aes_string(x=groupBy, y=value, colour=colourBy))+
  geom_boxplot()+ 
  ggtitle(bquote(Lineage~Ratio~Medians~(m["e,l"]*"-"*m["e.,."])~After~NaiveRUV2~Normalization~with ~Plate~Unit~of~Study))+
  xlab("")+ylab("")+
  #ylim(-6,4)+
  guides(fill=FALSE)+
  theme_grey(base_size = 12, base_family = "")+
