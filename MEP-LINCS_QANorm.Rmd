...
nYF <- nYF[!"LI8X00427"]
nYF <- nYF[!"LI8X00145"]
if(x[["Method"]]=="NaiveRandRUV"){
  nYF <- nYF[nYF$nu==0.1 & nYF$k == 2,]
  titleExpressionECMp= bquote(.(x[["Signal"]])~Medians~(m["e,l"]*"-"*m[".,l"])~After~.(x[["Method"]])~Normalization~with~k*"="~.(unique(nYF$k))*","~nu*"="~.(unique(nYF$nu)))
  titleExpressionLigand= bquote(.(x[["Signal"]])~Medians~(m["e,l"]*"-"*m["e,."])~After~.(x[["Method"]])~Normalization~with~k*"="~.(unique(nYF$k))*","~nu*"="~.(unique(nYF$nu)))
}  else if (x[["Method"]]=="RZS"){
  titleExpressionECMp= bquote(.(x[["Signal"]])~Medians~(m["e,l"]*"-"*m[".,l"])~After~.(x[["Method"]])~Normalization)
  titleExpressionLigand= bquote(.(x[["Signal"]])~Medians~(m["e,l"]*"-"*m["e,."])~After~.(x[["Method"]])~Normalization)
} else {
  nYF <- nYF[nYF$k == 2,]
  titleExpressionECMp= bquote(.(x[["Signal"]])~Medians~(m["e,l"]*"-"*m[".,l"])~After~.(x[["Method"]])~Normalization~with~k*"="~.(unique(nYF$k)))
  titleExpressionLigand= bquote(.(x[["Signal"]])~Medians~(m["e,l"]*"-"*m["e,."])~After~.(x[["Method"]])~Normalization~with~k*"="~.(unique(nYF$k)))
}
...
} else if (x[["Method"]]=="NaiveReplicateRUV"){
...
} else stop(paste(x[["Method"]]," is an unsupported normailization method"))

p <- ggplot(dt, aes(x=value, y=valueNormed, colour=k))+
  geom_point(alpha=.5)+
  facet_grid(Well~Barcode)+
  ggtitle(paste(x[["Method"]],"Normalized vs Raw",x[["Signal"]],chosenECMp,"replicate responses in",x[["CellLine"]]))
print(p)
...
p <- ggplot(dt, aes(x=valueNormed, fill=factor(k)))+
  ...
  ggtitle(paste(x[["Method"]],"Normalized",x[["Signal"]],chosenECMp,"replicate responses in",x[["CellLine"]]))
print(p)
