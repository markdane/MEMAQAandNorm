...
l3 <- l1[,list(Ligand=unique(Ligand), LigandAnnotID=unique(LigandAnnotID),
               ...
               MEP=unique(MEP),
               StainingSet=unique(StainingSet),
...
medianNaiveReplicateRUV2SS <- function(k, Y, cIdx, scIdx){
  ...
  nYm <- data.table(nYm)
  nYm <- nYm[, BWLSE := NULL]
  nYm <- nYm[,list(Value = median(value)), by="StainingSet,Barcode,MEP,ECMp,Ligand"]
  return(nYm)
}
...
madNaiveReplicateRUV2SS <- function(k, Y, cIdx, scIdx){
  ...
  nYm <- data.table(nYm)
  nYm <- nYm[, BWLSE := NULL]
  nYm <- nYm[,list(Value = mad(value)), by="StainingSet,Barcode,MEP,ECMp,Ligand"]
  return(nYm)
}
...
nYkList <- lapply(1:2, function(k, Y, cIdx, scIdx, mValue, MValue){
  ...
  #Debug fails here, may be due to Y being a data.table
   ...
   setnames(nYM, "Value", "Mel")
  setkey(nYm, StainingSet,Barcode,MEP,ECMp,Ligand)
  setkey(nYM, StainingSet,Barcode,MEP,ECMp,Ligand)
  nY <- merge(nYm, nYM)
  #Add margin values
  nY <- addMarginValues(dt=nY, mValue="mel",MValue="Mel")
...
```{r, eval=FALSE}
#eval=(x[["CellLine"]]=="PC3" & x[["StainingSet"]]=="SS2")|(x[["CellLine"]]=="MCF7" & x[["StainingSet"]]=="SS2")
...
} else stop(paste(unique(x[["Method"]])," is an unsupported normalization method"))
